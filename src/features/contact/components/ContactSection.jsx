...
function ChannelIcon({ type }) {
  switch (type) {
    case 'whatsapp':
      return (
        <span className="p-3 bg-gradient-to-tr from-green-100 to-emerald-200 text-green-700 rounded-2xl inline-flex items-center justify-center shrink-0 shadow-2xs border border-green-300/40">
    ...
    case 'shopee':
      return (
        <span className="p-3 bg-gradient-to-tr from-orange-100 to-amber-200 text-orange-600 rounded-2xl inline-flex items-center justify-center shrink-0 shadow-2xs border border-orange-300/40">
          <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
    ...
    case 'ebook':
      return (
        <span className="p-3 bg-gradient-to-tr from-purple-100 to-indigo-200 text-purple-700 rounded-2xl inline-flex items-center justify-center shrink-0 shadow-2xs border border-purple-300/40">
    ...
    case 'tiktok':
      return (
        <span className="p-3 bg-gradient-to-tr from-slate-100 to-gray-200 text-slate-900 rounded-2xl inline-flex items-center justify-center shrink-0 shadow-2xs border border-slate-300/40">
          <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
    ...
    case 'instagram':
      return (
        <span className="p-3 bg-gradient-to-tr from-pink-100 via-rose-100 to-purple-200 text-pink-600 rounded-2xl inline-flex items-center justify-center shrink-0 shadow-2xs border border-pink-300/40">
          <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
    ...
    case 'x':
      return (
        <span className="p-3 bg-gradient-to-tr from-sky-100 to-blue-200 text-sky-700 rounded-2xl inline-flex items-center justify-center shrink-0 shadow-2xs border border-sky-300/40">
          <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
    ...
    default:
      return (
        <span className="p-3 bg-gradient-to-tr from-blue-100 to-indigo-200 text-blue-700 rounded-2xl inline-flex items-center justify-center shrink-0 shadow-2xs border border-blue-300/40">
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function getChannelTheme(type) {
  switch (type) {
    ...
    case 'tiktok':
      return {
        topBorder: 'border-t-4 border-t-slate-800',
        badge: 'bg-slate-100 text-slate-900 border-slate-300',
        actionColor: 'text-slate-900 group-hover:text-black',
      };
    case 'instagram':
      return {
        topBorder: 'border-t-4 border-t-pink-500',
        badge: 'bg-pink-100 text-pink-900 border-pink-300',
        actionColor: 'text-pink-600 group-hover:text-pink-700',
      };
    case 'x':
      return {
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          {siteConfig.channels.map((channel, index) => {
            const channelTheme = getChannelTheme(channel.type);
            ...
            return (
              ...
              >
                <Card
                  hoverEffect
                  className={`h-full flex flex-col justify-between p-6 transition-all duration-300 rounded-3xl shadow-lg hover:shadow-2xl hover:-translate-y-2 border border-white/80 bg-white/98 backdrop-blur-xs ${channelTheme.topBorder}`}
                >
                  <div>
                    <div className="flex items-start justify-between gap-3 mb-4">
                      ...
                      {channel.badge && (
                        <span
                          className={`text-[11px] font-extrabold px-3 py-1 rounded-full border shadow-2xs ${channelTheme.badge}`}
                        >
                          {channel.badge}
                        </span>
                      )}
                    ...
                    <h3 className="text-lg font-bold text-gray-900 group-hover:text-blue-700 transition-colors mb-2 flex items-center gap-1.5">
                  ...
                  <div className="pt-3.5 border-t border-gray-100 flex items-center justify-between text-xs">
                    ...
                    <span className={`font-bold transition-transform group-hover:translate-x-1 ${channelTheme.actionColor}`}>
                      Kunjungi &rarr;
                    </span>
                  </div>
                </Card>
